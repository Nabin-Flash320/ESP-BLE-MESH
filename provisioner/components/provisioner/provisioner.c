                               ...
                               uint16_t unicast, uint8_t elem_num, uint16_t net_idx)
{
    ...
    provisioner_add_prov_dev_info(uuid, unicast, elem_num);
#warning("provisioned device info other operations")
                                ...
                                uint8_t adv_type, esp_ble_mesh_prov_bearer_t bearer)
{
    ...
    provisioner_add_unprov_dev_info(&add_dev);
    return;
}
                                              ...
                                              esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
        ...
        {
            ESP_LOGI(TAG, "composition data %s from 0x%04X", bt_hex(param->status_cb.comp_data_status.composition_data->data, param->status_cb.comp_data_status.composition_data->len), param->params->ctx.addr);
            parse_composition_data(param->status_cb.comp_data_status.composition_data, param->params->ctx.addr);
...
esp_err_t provisioner_init(void)
{
    ...
    ESP_LOGE(TAG, "BLE mesh provisioner started!!");
    return err;
}
...
int provisioner_add_dev_to_node(uint8_t idx)
{
    ...
    esp_ble_mesh_unprov_dev_add_t *device_to_add = get_unprov_dev(idx);
    if (!device_to_add)
    {
        ESP_LOGE(TAG, "Unable to get the device idx, please list the unprovisioned devices to get the valid idx");
        return -1;
    }

    esp_err_t err = esp_ble_mesh_provisioner_add_unprov_dev(device_to_add,
